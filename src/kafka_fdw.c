                        ...
                        int               eflags)
{
 ...
 static void
KafkaWriteAttributes_LUCENE(KafkaFdwModifyState *festate, TupleTableSlot *slot)
{
    ListCell *lc;
    ...
    foreach (lc, festate->attnumlist)
    {
        ...
        else
        {
         ...
         typoid = getBaseType(festate->typioparams[pindex]);

switch (typoid)
    {
        case BOOLOID: 
         appendStringInfoString(result, "BOOLOID");break;
        case INT2OID:
         appendStringInfoString(result, "INT2OID");break;
        case INT4OID:
         appendStringInfoString(result, "INT4OID");break;
        case INT8OID:
        appendStringInfoString(result, "INT8OID");break;
        case FLOAT4OID:
        appendStringInfoString(result, "FLOAT4OID");break;
        case FLOAT8OID:
         appendStringInfoString(result, "FLOAT8OID");break;
        case NUMERICOID:
            appendStringInfoString(result, "NUMERICOID");break;

        case DATEOID: 
        appendStringInfoString(result, "DATEOID");break;

        case TIMESTAMPOID: 
        appendStringInfoString(result, "TIMESTAMPOID");break;

        case TIMESTAMPTZOID: 
        appendStringInfoString(result, "TIMESTAMPTZOID");break;

        case JSONOID:
                appendStringInfoString(result, "JSONOID");break;

        case JSONBOID:
               appendStringInfoString(result, "JSONBOID");break;


        default:
            appendStringInfoString(result, "other");break;
        }
...
kafkaExecForeignInsert(EState *estate, ResultRelInfo *rinfo, TupleTableSlot *slot, TupleTableSlot *planSlot)
{
    ...
    {
        KafkaWriteAttributes_LUCENE(festate, slot);
    }
    jni_cmd1 = (*env)->NewStringUTF(env, festate->attribute_buf.data);
    ...
    if (slot != NULL && festate->attnumlist != NIL)
    {
        ...
        jni_cmd2 = (*env)->NewStringUTF(env, festate->attribute_buf.data);


 
  jni_mid=(*env)->GetMethodID(env,festate->jni_cls,"insert","(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");
    line = (jstring)(*env)->CallObjectMethod(env,festate->jni_obj, jni_mid,jni_cmd1,jni_cmd2);
